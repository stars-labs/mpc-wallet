...
/// Convert legacy CLI WalletData to shared FrostKeystoreData format
pub fn wallet_data_to_frost_keystore(
    wallet_data: &WalletData,
    wallet_info: &super::models::WalletInfo,
    device_info: &DeviceInfo,
) -> Result<FrostKeystoreData> {
    // Determine curve type and serialize packages
    let (key_package_base64, public_key_base64, curve) = if let Some(key_pkg) = &wallet_data.secp256k1_key_package {
        let key_bytes = serde_json::to_vec(key_pkg)
            .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
        let pub_bytes = serde_json::to_vec(wallet_data.secp256k1_public_key.as_ref()
            .ok_or_else(|| KeystoreError::General("Missing secp256k1 public key".into()))?)
            .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
    ...
    } else if let Some(key_pkg) = &wallet_data.ed25519_key_package {
        let key_bytes = serde_json::to_vec(key_pkg)
            .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
        let pub_bytes = serde_json::to_vec(wallet_data.ed25519_public_key.as_ref()
            .ok_or_else(|| KeystoreError::General("Missing ed25519 public key".into()))?)
            .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
        ...
        return Err(KeystoreError::General("No key package found".into()));
    };
    ...
    let participant_index = wallet_info.devices
        .iter()
        .position(|d| d.device_id == device_info.device_id)
        .map(|i| i as u16 + 1) // Convert to 1-based index
        .unwrap_or(1);
    
    let participant_indices = (1..=wallet_info.total_participants).collect();
    
    Ok(FrostKeystoreData {
        key_package: key_package_base64,
        public_key_package: public_key_base64,
        min_signers: wallet_info.threshold,
        max_signers: wallet_info.total_participants,
        participant_index,
        participant_indices,
        curve,
        wallet_id: Some(wallet_info.wallet_id.clone()),
        device_id: Some(device_info.device_id.clone()),
        device_name: Some(device_info.name.clone()),
        session_id: Some(wallet_data.session_id.clone()),
        timestamp: Some(Utc::now().to_rfc3339()),
    })
}

/// Convert shared FrostKeystoreData to legacy CLI WalletData format
pub fn frost_keystore_to_wallet_data(
    keystore_data: &FrostKeystoreData,
) -> Result<WalletData> {
    let device_id = keystore_data.device_id.clone()
        .unwrap_or_else(|| format!("device_{}", keystore_data.participant_index));
    let session_id = keystore_data.session_id.clone()
        .unwrap_or_else(|| keystore_data.wallet_id.clone()
            .unwrap_or_else(|| uuid::Uuid::new_v4().to_string()));
    
    match keystore_data.curve.as_str() {
        "secp256k1" => {
            let key_bytes = general_purpose::STANDARD.decode(&keystore_data.key_package)
                .map_err(|e| KeystoreError::DecryptionError(e.to_string()))?;
            let pub_bytes = general_purpose::STANDARD.decode(&keystore_data.public_key_package)
                .map_err(|e| KeystoreError::DecryptionError(e.to_string()))?;
            
            let key_package: Secp256k1KeyPackage = serde_json::from_slice(&key_bytes)
                .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
            let public_key: Secp256k1PublicKeyPackage = serde_json::from_slice(&pub_bytes)
                .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
        ...
        "ed25519" => {
            ...
            let key_package: Ed25519KeyPackage = serde_json::from_slice(&key_bytes)
                .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
            let public_key: Ed25519PublicKeyPackage = serde_json::from_slice(&pub_bytes)
                .map_err(|e| KeystoreError::SerializationError(e.to_string()))?;
        ...
        _ => Err(KeystoreError::UnsupportedBlockchain(keystore_data.curve.clone())),
    }
}
