<script lang="ts">
    ...
    const dispatch = createEventDispatcher<{backToWallet: {chain: string, curve: string}}>(); 
    ...
    onMount(async () => {
        ...
        try {
            ...
            const message = await chrome.runtime.sendMessage({ type: "getState" });
            if (message && message.blockchain) {
                chain = message.blockchain;
                curve = chain === "ethereum" ? "secp256k1" : "ed25519";
            }
            
            // Get networks for current chain
            const chainNetworks = networkService.getNetworks(chain);
            if (Array.isArray(chainNetworks)) {
                networks = chainNetworks;
                currentNetwork = networkService.getCurrentNetwork(chain);
            } else {
                console.error("[Settings] Networks is not an array:", chainNetworks);
                networks = [];
            }
        } catch (error) {
    ...
    function handleChainChange() {
        ...
        try {
            // Update networks list when chain changes
            const chainNetworks = networkService.getNetworks(chain);
            if (Array.isArray(chainNetworks)) {
                networks = chainNetworks;
                currentNetwork = networkService.getCurrentNetwork(chain);
        ...
        chrome.runtime.sendMessage(
            ...
            (response) => {
                if (chrome.runtime.lastError) {
                    ...
                    console.log("[Settings] Blockchain selection saved:", chain);
                    // Only update locally without closing settings
                    // We don't want to automatically return to wallet page anymore
                    console.log('[Settings] Blockchain selection saved locally:', chain);
                }
            }
        );
    }
    ...
    async function handleNetworkChange(event: Event) {
        ...
        try {
            await networkService.setCurrentNetwork(chain, networkId);
            currentNetwork = networkService.getCurrentNetwork(chain);
        } catch (error) {
    ...
    async function addCustomNetwork() {
        if (!customNetworkName || !customNetworkRpcUrl || !customNetworkChainId || !customNetworkSymbol) {
        ...
        try {
            ...
            await networkService.addNetwork(chain, customChain);
            
            // Refresh the networks list
            networks = networkService.getNetworks(chain) as Chain[];
            
            // Switch to the new network
            await networkService.setCurrentNetwork(chain, chainId);
            currentNetwork = networkService.getCurrentNetwork(chain);
...
<div class="p-4">
    <div class="flex justify-between items-center mb-4">
        ...
        <button class="text-sm px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600" on:click={() => dispatch('backToWallet', {chain, curve})}>Back to Wallet</button>
    </div>
    ...
    <div class="mb-6 p-4 border rounded">
        ...
        <!-- Curve Selection -->
        <div class="mb-4">
            <label for="curve-select" class="block font-bold mb-2">Curve Type:</label>
            <select
                id="curve-select"
                bind:value={curve}
        ...
        <!-- Network Selection (for Ethereum) -->
        {#if chain === "ethereum" && networks.length > 0}
            <div class="mb-4">
                <label for="network-select" class="block font-bold mb-2">Network:</label>
                <select
                    id="network-select"
                ...
                >
                    {showCustomNetworkForm ? 'Hide Custom Network Form' : 'Add Custom Network'}
                </button>
            </div>
            ...
            {#if currentNetwork}
                <div class="mt-2 p-3 bg-gray-50 rounded text-sm">
                    ...
                    {#if currentNetwork.rpcUrls?.default?.http}
                        <p><strong>RPC URL:</strong> {currentNetwork.rpcUrls.default.http[0]}</p>
                    {/if}
                </div>
            {/if}
        {/if}
    ...
    <div class="mb-6 p-4 border rounded">
        ...
        <div class="mb-4">
            <label for="network-name" class="block font-bold mb-2">Network Name:</label>
            <input
                id="network-name"
                type="text"
                class="w-full border p-2 rounded"
        ...
        <div class="mb-4">
            <label for="network-rpc" class="block font-bold mb-2">RPC URL:</label>
            <input
                id="network-rpc"
                type="text"
                class="w-full border p-2 rounded"
        ...
        <div class="mb-4">
            <label for="network-chainid" class="block font-bold mb-2">Chain ID:</label>
            <input
                id="network-chainid"
                type="text"
                class="w-full border p-2 rounded"
        ...
        <div class="mb-4">
            <label for="network-symbol" class="block font-bold mb-2">Currency Symbol:</label>
            <input
                id="network-symbol"
                type="text"
                class="w-full border p-2 rounded"
        ...
        <div class="mb-4">
            <label for="network-explorer" class="block font-bold mb-2">Block Explorer URL: (optional)</label>
            <input
                id="network-explorer"
                type="text"
                class="w-full border p-2 rounded"
