...
export class WebRTCManager {
  ...
  private async _getOrCreatePeerConnection(peerId: string): Promise<RTCPeerConnection | null> {
    ...
    pc.onconnectionstatechange = () => {
      ...
      // Only use the callback, not sendToBackground
      this.onWebRTCConnectionUpdate(peerId, connected);

      if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected' || pc.connectionState === 'closed') {
        this._log(`Connection to ${peerId} failed/disconnected/closed. Cleaning up.`);
  ...
  private _setupDataChannelHandlers(dc: RTCDataChannel, peerId: string): void {
    ...
    dc.onerror = (event) => {
      this._log(`Data channel error with ${peerId}: ${event}`);
    ...
    dc.onmessage = (event) => {
      this._log(`Received data from ${peerId}: ${event.data}`);
      try {
  ...
  // Add missing method to start session
  ...
  // Add missing method to accept session  
