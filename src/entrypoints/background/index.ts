...
export default defineBackground(() => {
                        ...
                        if (validateSessionAcceptance(message)) {
                            ...
                            if (sessionIndex >= 0) {
                                ...
                                if (message.accepted) {
                                    ...
                                    // Send acceptance message to proposer/participants
    ...
    function handleSessionResponse(fromPeerId: string, responseData: any) {
        ...
        const { session_id, accepted, peer_id } = responseData;

        // Find the session in our current sessionInfo
        if (appState.sessionInfo && appState.sessionInfo.session_id === session_id) {
            console.log("[Background] Session response for current session:", session_id);

            if (accepted) {
                // Add peer to accepted peers list if not already there
                if (!appState.sessionInfo.accepted_peers.includes(peer_id)) {
                    appState.sessionInfo.accepted_peers.push(peer_id);
                    console.log("[Background] Added peer to accepted list:", peer_id);
                }
            } else {
                // Remove peer from participants if they declined
                appState.sessionInfo.participants = appState.sessionInfo.participants.filter(p => p !== peer_id);
                console.log("[Background] Removed declining peer from participants:", peer_id);
            }
            ...
            console.log("[Background] Session response processed and broadcasted to popup");
        } else {
    ...
    function handleSessionProposal(fromPeerId: string, proposalData: any) {
        ...
        if (isParticipant) {
            ...
            // Also broadcast as a proposal message for popup to handle
            broadcastToPopupPorts({
                type: "proposeSession",
