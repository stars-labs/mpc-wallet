...
describe('NetworkService', () => {
    ...
    beforeEach(async () => {
        ...
        await networkService.setCurrentNetwork('ethereum', 1); // Set mainnet as current
    });
    ...
    it('should throw error when updating protected networks', async () => {
        // First need to add mainnet since we mocked getDefaultNetworks
        const mainnet: Chain = {
            id: 1,
        ...
        await networkService.addNetwork('ethereum', mainnet);
    ...
    it('should clear networks while preserving protected ones', async () => {
        // Add mainnet and sepolia first (protected networks)
        const mainnet: Chain = {
            id: 1,
            name: 'Ethereum Mainnet',
            network: 'mainnet',
        ...
        await networkService.addNetwork('ethereum', mainnet);
        await networkService.addNetwork('ethereum', sepolia);
    ...
    it('should get current network without blockchain parameter', async () => {
        ...
        // Set current networks for both blockchains
    ...
    it('should get public client for ethereum networks', async () => {
        ...
        // Set current blockchain to ethereum
        await networkService.setCurrentBlockchain('ethereum');
    ...
    it('should throw error when getting public client without current ethereum network', async () => {
        // Set blockchain to ethereum but no current network
        await networkService.setCurrentBlockchain('ethereum');
        // Don't set a current network
    ...
    it('should handle network removal with current network fallback', async () => {
        // Add mainnet and custom network
        const mainnet: Chain = {
            id: 1,
            name: 'Ethereum Mainnet',
            network: 'mainnet',
    ...
    it('should throw error when updating protected networks', async () => {
        // First ensure we have the default networks loaded
        const defaultNetworks = await networkService.getNetworks('ethereum') as Chain[];
        if (defaultNetworks.length === 0) {
            // Add mainnet for testing
            const mainnet: Chain = {
                id: 1,
                name: 'Ethereum Mainnet',
                network: 'mainnet',
    ...
    it('should clear networks while preserving protected ones', async () => {
        ...
        // Should only have protected networks (mainnet, sepolia, etc.)
        expect(ethNetworks.find((n: Chain) => n.id === 888)).toBeUndefined();
        expect(ethNetworks.find((n: Chain) => n.id === 999)).toBeUndefined();
        expect(ethNetworks.find((n: Chain) => n.id === 1)).toBeDefined(); // mainnet
        expect(ethNetworks.find((n: Chain) => n.id === 11155111)).toBeDefined(); // sepolia
    });
    ...
    it('should handle network removal with current network fallback', async () => {
        // Add a custom network and set it as current
        const customNetwork: Chain = {
            id: 888,
            name: 'Test Network',
            nativeCurrency: { name: 'TST', symbol: 'TST', decimals: 18 },
